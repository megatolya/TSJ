...
module.exports = {
    ...
    checkAdmin: function (req, res, next) {
        logger.info('checking admin info');
    ...
    flat: function (req, res) {
        ...
        api.flat(id, function(err, flat){
            ...
            models.month.find({flat: id}, function (err, payments) {
                var tasks = [];

                payments.forEach(function (payment) {
                    tasks.push(function (callback) {
                        api.month(payment._id, function (err, payment) {
                            callback(null, payment);
                        });
                    });
                });
                async.parallel(tasks, function (err, payments) {
                    res.render('flat', {
                        data: getDefaults(req),
                        tab: 0,
                        users: flat.users,
    ...
    changeStatusOfMsg : function (req, res) {
        models.msg.findById(req.body.id, function(err, msg) {
            if (err)
                return res.render(500);
